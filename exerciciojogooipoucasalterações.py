...
class Jogador:
    ...
    def debitar_saldo(self, quantidade):
        saldo = self._saldo_carteira
        if saldo < quantidade:
            print("Não foi possivel fazer a operação ai.\nSaldo disponível: ", self._saldo_carteira)
        else:
    ...
    def set_nome_jogador(self, saldo):
        self._saldo_carteira = saldo
...
class Plataforma:  
    ...
    def exibir_tudo(self):
        jogos = self._catalogo
        #jogadores_nomes = [jogador.get_nickname]
        #print(f"Jogadores cadastrados:")
        #for jogador in self._jogadores_cadastrados:
        #    print(jogadores_nomes)
        for jogo in jogos:
...
oi = Jogador("nonn", "#12908765433", 70)
oi.debitar_saldo(12)
oi.exibir_perfil()
